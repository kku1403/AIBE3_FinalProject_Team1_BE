...
import com.back.domain.reservation.dto.AuthorDto;
import com.back.domain.reservation.dto.CreateReservationReqBody;
import com.back.domain.reservation.dto.GuestReservationSummaryResBody;
import com.back.domain.reservation.dto.HostReservationSummaryResBody;
import com.back.domain.reservation.dto.OptionDto;
import com.back.domain.reservation.dto.ReservationDto;
import com.back.domain.reservation.dto.ReservationLogDto;
import com.back.domain.reservation.dto.ReservationStatusResBody;
...
public class ReservationService {
	...
	public ReservationDto create(CreateReservationReqBody reqBody, Member author) {
		...
		notificationService.saveAndSendNotification(post.getAuthor().getId(),
			NotificationType.RESERVATION_PENDING_APPROVAL, r.getId());
	...
	private void validateDeliveryMethods(Post post, ReservationDeliveryMethod receiveMethod,
	...
	public PagePayload<GuestReservationSummaryResBody> getSentReservations(
		...
		Page<Reservation> reservationPage = reservationQueryRepository.findByAuthorWithFetch(author, status, keyword,
			pageable);
		...
		Set<Long> reviewedReservationIds = reviewQueryRepository.findReviewedReservationIds(reservationIds,
			author.getId());
	...
	private GuestReservationSummaryResBody.ReservationPostSummaryDto createPostSummaryDto(Post post) {
		AuthorDto authorDto =
			new AuthorDto(
				...
				s3.getProfileThumbnailUrl(post.getAuthor().getProfileImgUrl())
			);
	...
	public PagePayload<HostReservationSummaryResBody> getReceivedReservations(
		...
		Page<HostReservationSummaryResBody> reservationSummaryDtoPage = reservationPage.map(reservation -> {
			...
			String profileImgUrl = s3.getProfileThumbnailUrl(reservation.getAuthor().getProfileImgUrl());
	...
	public ReservationDto updateReservationStatus(Long reservationId, Long memberId,
		UpdateReservationStatusReqBody reqBody) {
	...
	private void updateClaimingStatus() {
		List<Reservation> reservations = reservationQueryRepository.findWithPostAndAuthorByStatus(
			ReservationStatus.CLAIMING);

		reservations.forEach(reservation -> {
			reservation.changeStatus(ReservationStatus.CLAIM_COMPLETED);
			notificationService.saveAndSendNotification(reservation.getAuthor().getId(),
				NotificationType.RESERVATION_CLAIM_COMPLETED, reservation.getId());
			notificationService.saveAndSendNotification(reservation.getPost().getAuthor().getId(),
				NotificationType.RESERVATION_CLAIM_COMPLETED, reservation.getId());
		});
	...
	private void updatePendingRefundStatus() {
		...
		reservations.forEach(reservation -> {
			...
			notificationService.saveAndSendNotification(reservation.getAuthor().getId(),
				NotificationType.RESERVATION_REFUND_COMPLETED, reservation.getId());
			notificationService.saveAndSendNotification(reservation.getPost().getAuthor().getId(),
				NotificationType.RESERVATION_REFUND_COMPLETED, reservation.getId());
		});
